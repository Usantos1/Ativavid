...
- **`render_queue.py`** (renderiza a Fase 2 de VARIOS projetos ao mesmo tempo —
  e assim que se ganha vazao: o Remotion recusa concurrency acima do numero de
  nucleos, entao um render sozinho nunca usa mais do que a maquina tem. A fila
  divide os nucleos entre os jobs. **Sempre rode com `--dry-run` primeiro**: sem
  ele o comando renderiza de verdade e sobrescreve `remotion/out/render.mp4` de
  cada projeto da lista.)
...
**A legenda aparece SOBRE o video na aba Inicio, antes de renderizar** (botao
`legenda` no canto do player, ligado por padrao). O laco caro era: renderiza
(~100s num reel curto, minutos num longo) -> ve a palavra errada -> corrige ->
renderiza tudo de novo. Agora a palavra errada aparece antes de qualquer
render, e a trilha de legenda tambem fica visivel na Fase 1 justamente pra o
...
1. **Inventory.** `ffprobe` every source. `transcribe_batch.py` (or `transcribe.py`) → `pack_transcripts.py` → read `takes_packed.md`. Note dimensions/orientation and whether it looks flat/LOG. Material you can't picture from the transcript → `watch_video.py` for a one-Read visual survey.
