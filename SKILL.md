...
Phase 1:
- **`transcribe.py <video> --edit-dir <edit> [--language pt]`** — Groq Whisper, word-level, cached. Long audio auto-chunks; chunks fetch **in parallel** with per-chunk resume cache and 5x backoff retries (Groq blips don't restart the job).
...
- **`render.py <edl.json> -o cut.mp4 --no-subtitles [--voice-master] [--keep-resolution] [--jobs N]`** — per-segment extract (grade + fades, **parallel**) → lossless concat → optional voice master → loudnorm.
