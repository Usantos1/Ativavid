...
def _system_prompt(preset: dict | None = None) -> str:
    ...
    return (
        ...
        "- Também devolva headline curta (máx 6 palavras, SEM emoji — a fonte "
        "do vídeo não desenha emoji) em \"headline\" E mais "
...
def plan_cut(
    *,
    ...
    from app.llm_session import status as sess_status

    st = sess_status()
    if not st.get("ok"):
        # Sem sessão web NENHUMA capturada. Com a chave do Groq no .env o
        # plano ainda sai — sem ela, o erro de sempre.
        from app import llm_gateway as gw
